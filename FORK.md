...
A GitHub Action (`.github/workflows/sync-fork.yml`) runs every 2 hours to:

1. Check if upstream has new commits
2. If yes, sync `dev` with upstream
3. Rebase `rmk` onto the updated `dev`
4. Force push both branches

You can also trigger this manually via the Actions tab.
...
The workflow will keep your patches rebased on upstream automatically.
