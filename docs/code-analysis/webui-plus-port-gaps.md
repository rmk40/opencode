...
#### B1. `packages/app/src/components/titlebar.tsx` — iOS PWA safe-area + center portal hardening
...
- **Severity**: Major. **Restoration**: small. Depends on `Spinner` (already imported in current header), `createMediaQuery` (already a dep), `useGlobalSDK` (B3 must land first).

#### B3. `packages/app/src/context/global-sdk.tsx` — `reconnecting` signal, `restart()`, `isTouchDevice` strategy
...
#### B4. `packages/app/src/context/sync.tsx` — force-refresh ancillary reconciliation
...
#### B5. `packages/app/src/pages/session.tsx` — mobile session/changes pill portaled to titlebar
...
- **`hasUserPrompt` placeholder latch (commit `21f6c28c9`)**: plus latches `hasUserPrompt` to true once it's been true for the current session and pins until the session id changes. The current memo recomputes from scratch on every dep tick and can transiently flip back to false during SSE updates, flashing the placeholder back to the cycling example for one frame.
- **Inline +/send buttons (commit `0842f5ead`)**: plus puts the `+` button, the textarea, and the send button on the **same horizontal row** with `flex items-center gap-1 px-2 py-2` — vertically centered, no absolute positioning, no fade gradient. Current still uses absolute positioning (`absolute bottom-2 right-2` / `absolute bottom-2 left-2`) with a fade gradient over the textarea, which is an older upstream layout.
- **`pb-1.5`/`pt-4` mobile spacing (commit `11ddf8f5e`)**: plus has tightened spacing on the mode strip (`pt-4 pb-1.5` vs current `pt-5.5 pb-2`).
...
#### B8. `packages/app/src/index.css` — calmer progress, content-visibility gate, iOS PWA viewport, mobile bash mask
...
#### B9. `packages/ui/src/components/message-part.css` + `message-part.tsx` — mobile bash output cap with fade
...
| ----------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ----------- | ------------------------------- | --------------------------------------------------------------------------- |
| `e64377ca5`                                                             | fix(ios): remove nested pointer-events toggle in titlebar center                  | yes         | **Out**                         | B1                                                                          |
| `f4a78a87f`                                                             | fix(ios-pwa): prevent Changes pill width mutation from bricking titlebar          | yes         | **Out**                         | B5 (intentional static label)                                               |
| `eb3973f9c`                                                             | fix(mobile): constrain titlebar center so pill cannot overlap right slot          | yes         | **Out**                         | B1                                                                          |
| `d27d9e0e4`                                                             | fix(mobile): mobile pill tap target size and iOS tap-delay                        | yes         | **Out**                         | B5                                                                          |
| `5a86c0320`                                                             | fix(sync): reconcile session_status/todo/diff on forced refresh                   | yes         | **Out**                         | B4                                                                          |
| `864987261`                                                             | fix: dual-review follow-ups on today's three commits                              | yes         | **Out**                         | rolled into B1/B5/B8                                                        |
| `a9ade1b73`                                                             | fix(mobile): resume-sync + tighter heartbeat + refresh button                     | yes         | **Out**                         | B2 + B3                                                                     |
| `2d497346a`                                                             | fix(mobile): calmer progress indicator and self-heal stuck state                  | yes         | **Out**                         | B8 + B13                                                                    |
| `f5f075893`                                                             | fix(terminal): proxy WebSocket /pty/\*/connect through raw TCP                    | no          | OOS                             | server proxy                                                                |
| `923895987`                                                             | fix(pwa): use 100lvh in iOS PWA standalone mode                                   | yes         | **Out**                         | B8                                                                          |
| `96747006d`                                                             | fix(pwa): force titlebar stacking context above iOS translucent status bar        | yes         | **Out**                         | B1                                                                          |
| `39a2e7637`                                                             | fix(pwa): apply iOS safe-area insets at edges, not whole shell                    | yes         | **Out**                         | B1 + B11                                                                    |
| `3f6f413ca`                                                             | fix(mobile): bash-output mask works when collapsible expanded later               | yes         | **Out**                         | B9                                                                          |
| `84cb51933`                                                             | fix: address second-pass review findings on rendering commits                     | yes         | **Out**                         | rolled into B8/B9                                                           |
| `9996db665`                                                             | fix(pwa): add mobile-web-app-capable meta tag                                     | yes         | **Out**                         | B16                                                                         |
| `b969a29e4`                                                             | fix: read titlebar portal mounts synchronously to avoid layout shift              | yes         | **Out**                         | B2 + B5                                                                     |
| `2a11cb63c`                                                             | fix(pwa): handle iOS safe-area for translucent status bar                         | yes         | **Out**                         | B1                                                                          |
| `db414bf27`                                                             | fix(mobile): gate content-visibility on pointer capability not width              | yes         | **Out**                         | B8                                                                          |
| `562984401`                                                             | fix(mobile): cap bash output at 60vh with fade indicator                          | yes         | **Out**                         | B9                                                                          |
| `ec2d07a55`                                                             | fix(mobile): show full bash tool output on narrow viewports                       | yes         | **Out**                         | B9                                                                          |
| `96bf793ec`                                                             | fix(mobile): disable content-visibility:auto on narrow viewports                  | yes         | **Out**                         | superseded by `db414bf27` (B8)                                              |
| `e66ee1181`                                                             | fix(mobile): collapse todo dock by default on narrow viewports                    | yes         | **Out**                         | B10                                                                         |
| `21f6c28c9`                                                             | fix(prompt): latch hasUserPrompt to stop placeholder flashing                     | yes         | **Out**                         | B7                                                                          |
| `b724285a3`                                                             | fix(ios): stabilize layout during iPhone scroll/streaming                         | yes         | **Out**                         | B8 (content-visibility gate)                                                |
...
| `f89d9034f`                                                             | fix(mobile): reduce turn list bottom padding pb-16 → pb-4                         | yes         | **Out**                         | B13                                                                         |
| `c9517a7dd`                                                             | fix(mobile): vertically center + and send buttons in input row                    | yes         | **Out**                         | B7                                                                          |
| `11ddf8f5e`                                                             | fix(mobile): tighten input area spacing                                           | yes         | **Out**                         | B7                                                                          |
...
| `0842f5ead`                                                             | feat(mobile): inline + and send buttons with prompt input text                    | yes         | **Out**                         | B7                                                                          |
| `bdd5aab47`                                                             | fix: gate session-header center portal on md+ breakpoint                          | yes         | **Out**                         | B2                                                                          |
| `c9d53ddfb`                                                             | feat(mobile): move session/changes tabs into titlebar center as segmented control | yes         | **Out**                         | B5 (headline)                                                               |
| `3f1d38691`                                                             | fix: update manifest in packages/ui source and app/public                         | yes         | **Out**                         | B15                                                                         |
| `6e7afb9e6`                                                             | feat: PWA iOS support and reduce prompt input padding                             | yes         | **Out**                         | B16 + B11/B7                                                                |
...
| `f49909844`                                                             | fix: remove dev debug bar overlay                                                 | yes         | **Out**                         | B6                                                                          |
