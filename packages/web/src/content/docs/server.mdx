...
```bash
opencode serve [--port <number>] [--hostname <string>] [--cors <origin>] [--allow-insecure-no-auth]
```
...
| `--allow-insecure-no-auth` | Allow non-loopback binds without basic auth. This is unsafe.   | `false`          |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `opencode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to `opencode`, `opencode serve`, `opencode web`, and `opencode acp` when they bind an HTTP server.

```bash
OPENCODE_SERVER_PASSWORD=your-password opencode serve
```

Loopback binds (`localhost`, `127.0.0.1`, and `::1`) may run without a password for local development. Non-loopback binds, including `0.0.0.0` and mDNS defaults, refuse to start unless `OPENCODE_SERVER_PASSWORD` is set. Use `--allow-insecure-no-auth` or `server.allowInsecureNoAuth` only when you intentionally accept unauthenticated network access.
