...
:::caution
Loopback web servers are allowed without a password for local use. Set `OPENCODE_SERVER_PASSWORD` before binding to a non-loopback host such as `0.0.0.0` or enabling mDNS.
:::
...
Because mDNS binds to the network, it also requires `OPENCODE_SERVER_PASSWORD` unless you intentionally pass `--allow-insecure-no-auth`.
...
For automation where unauthenticated network access is intentional, pass `--allow-insecure-no-auth` or set `server.allowInsecureNoAuth`. This exposes the server without authentication and should not be used on untrusted networks.
